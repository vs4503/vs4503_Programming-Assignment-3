...
{ 
  ...
  //Code to find bounding rectangle 
  let x = min(x0, x1, x2);
  let y = min(y0, y1, y2);
  ...
  //Loop through all points within bounding rectangle
  ...
  function edgeFunction(i,j){
    ...
    //calculate area of triangle using edge function
    ...
    if(lambda0 >= 0 && lambda1 >= 0 && lambda2 >= 0){
      ...
      //Assign color to pixel based on the barycentric coordinates
      r = lambda0*r0 + lambda1*r1 + lambda2*r2;
      g = lambda0*g0 + lambda1*g1 + lambda2*g2;
      b = lambda0*b0 + lambda1*b1 + lambda2*b2;
